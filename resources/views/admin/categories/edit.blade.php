...
@section('content')
<form action="{{route('categories.update', $category)}}" method="POST" class="mx-auto max-w-3xl">
    @method('patch')
    @csrf
    <div class="bg-white flex flex-col p-6 rounded-xl border-4 border-blue-400">
        <div class="items-center p-3 @error('name') flex-col @enderror justify-center">
            <div class="flex flex-col md:flex-row grow justify-between">
                ...
                <input type="text" id="name" class="border-2 border-gray-300 md:w-4/5 w-full  rounded-lg @error('name') is-invalid @enderror" name="name" value="{{$category->name}}">
            </div>
        ...
        <div class="flex flex-col md:flex-row grow justify-between">
            ...
            <textarea name="description" id="description" class="border-2 border-gray-300 md:w-4/5 w-full rounded-lg @error('description') is-invalid @enderror p-2 h-24">{{$category->description}}</textarea>
            </div>
            <div class="flex justify-center mt-3">
                @error('description')
                <span>
