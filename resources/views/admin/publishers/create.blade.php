...
@section('content')
<form action="{{route('admin.publishers.store')}}" method="POST" class="mx-auto max-w-3xl">
    @csrf
    <div class="bg-white flex flex-col p-6 rounded-xl border-4 border-blue-400">
        ...
        <div class="flex flex-col md:flex-row grow justify-between">
            ...
            <textarea name="address" id="address" class="border-2 border-gray-300 md:w-4/5 w-full rounded-lg @error('address') is-invalid @enderror p-2 h-24"></textarea>
            </div>
            <div class="flex justify-center mt-3">
                @error('address')
                <span>
